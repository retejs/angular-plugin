...
export function setup<Schemes extends ClassicScheme, K extends AngularArea2D<Schemes>>(
  ...
  return {
    ...
    mount(context, plugin) {
      ...
      const rendered = () => {
        emit({ type: 'rendered', data: context.data } as K)
      }
      ...
      if (context.data.type === 'connection') {
        ...
        return {
          ...
          props: {
            ...
            data: payload,
            start: start || ((change: (value: Position) => void) => positionWatcher.listen(source, 'output', sourceOutput, change)),
            end: end || ((change: (value: Position) => void) => positionWatcher.listen(target, 'input', targetInput, change)),
